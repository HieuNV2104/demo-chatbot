...
const OperationBtn = ({
    ...
    <div className={`${s['operation-btn']} ${className ?? ''}`} onClick={onClick && onClick}>
        {innerContent}
    </div>
);
...
const RatingIcon: FC<{ isLike: boolean }> = ({ isLike }) => {
    return isLike ? (
        <HandThumbUpIcon className={`${s['hand-icon']}`} />
    ) : (
        <HandThumbDownIcon className={`${s['hand-icon']}`} />
    );
};
...
const IconWrapper: FC<{ children: React.ReactNode | string }> = ({ children }) => {
    return <div className={`${s['icon-wrapper']}`}>{children}</div>;
};
...
const Answer: FC<IAnswerProps> = ({
    ...
    const renderFeedbackRating = (rating: MessageRating | undefined) => {
        ...
        const ratingIconClassname = isLike ? s['is-like'] : s['is-dislike'];
        // The tooltip is always displayed, but the content is different for different scenarios.
        return (
            <Tooltip selector={`user-feedback-${randomString(16)}`} content={isLike ? 'like' : 'dislike'}>
                <div
                    className={`${s.tooltip}`}
                    onClick={async () => {
                ...
                >
                    <div className={`${ratingIconClassname} ${s['tooltip-child']}`}>
    ...
    const renderItemOperation = () => {
        ...
        return (
            <div style={{ display: 'flex', gap: '0.5rem' }} className={`${s.itemOperation}`}>
                {userOperation()}
            </div>
        );
    };
            ...
            <div className={`${s['answer-box']}`}>
                ...
                <div className={`${s.answerWrap}`}>
                    <div className={`${s.answer}`}>
                        <div style={workflowProcess && { minWidth: '480px' }} className={`${s['answer-child']}`}>
                            {workflowProcess && <WorkflowProcess data={workflowProcess} hideInfo />}
                            {isResponding &&
                            (isAgentMode
                                ? !content &&
                                  (agent_thoughts || []).filter((item) => !!item.thought || !!item.tool).length === 0
                                : !content) ? (
                                <div className={`${s['answer-loading']}`}>
                        ...
                        <div className={`${s.feedback} ${feedback?.rating ? s['has-rating'] : ''}`}>
