...
import HistoryChat from '@/components/layouts/history-chat/history-chat';
...
const Main: FC = () => {
     ...
     * app info
     */
    const [historyChatList, setHistoryChatList] = useState<any | null>(null);
    ...
    const [conversationIdChangeBecauseOfNew, setConversationIdChangeBecauseOfNew, getConversationIdChangeBecauseOfNew] =
        useGetState(false);
    ...
    // get history chat
    const getHistoryChat = async (currConversationId: string) => {
        try {
            const data = await fetchChatList(currConversationId);
    ...
    useEffect(() => {
        currConversationId !== '-1' && getHistoryChat(currConversationId);
    }, [currConversationId]);
    //
    useEffect(() => {
        // scroll to bottom
    ...
    return (
        <>
            <main ref={chatListDomRef} className={`${MainStyles.main}`}>
                <Chat
                    chatList={chatList}
                    onSend={handleSend}
