...
const TemplateVarPanel: FC<ITemplateVarPanelProps> = ({ className, header, children, isFold }) => {
    return (
        <div className={cn(isFold ? 'border border-indigo-100' : s.boxShodow, className, 'rounded-xl ')}>
            {/* header */}
            <div className={cn(isFold && 'rounded-b-xl', 'rounded-t-xl px-6 py-4 bg-indigo-25 text-xs')}>{header}</div>
            {/* body */}
            {!isFold && children && <div className="rounded-b-xl p-6">{children}</div>}
        </div>
    );
};
