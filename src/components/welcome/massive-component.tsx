...
export const PromptTemplate: FC<{ html: string }> = ({ html }) => {
    return <div className={' box-border text-sm text-gray-700'} dangerouslySetInnerHTML={{ __html: html }}></div>;
};
...
export const ChatBtn: FC<{ onClick: () => void; className?: string }> = ({ className, onClick }) => {
    const { t } = useTranslation();
    return (
        <Button
            type="primary"
...
export const EditBtn = ({ className, onClick }: { className?: string; onClick: () => void }) => {
    ...
    return (
        <div className={cn('px-2 flex space-x-1 items-center rounded-md  cursor-pointer', className)} onClick={onClick}>
