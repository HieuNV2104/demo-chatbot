...
const Base: FC<Props> = ({ className, title, headerRight, children, minHeight = 120, value, isFocus }) => {
    const ref = useRef<HTMLDivElement>(null);
    const { wrapClassName, isExpand, setIsExpand, editorExpandHeight } = useToggleExpend({ ref, hasFooter: false });
        ...
        <div className={cn(wrapClassName)}>
            <div
                ...
                className={cn(
                    ...
                    isExpand && 'h-full',
                    'rounded-lg border',
                    isFocus ? 'bg-white border-gray-200' : 'bg-gray-100 border-gray-100 overflow-hidden'
                )}
            >
