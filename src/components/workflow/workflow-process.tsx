...
const WorkflowProcessItem = ({ data, grayBg, expand = false, hideInfo = false }: WorkflowProcessProps) => {
    ...
    return (
        ...
        >
            <div
                className={cn('flex items-center h-[18px] cursor-pointer', hideInfo && 'px-[6px]')}
                onClick={() => setCollapse(!collapse)}
            >
                {running && <Loading02 className="shrink-0 mr-1 w-3 h-3 text-[#667085] animate-spin" />}
                {succeeded && <CheckCircle className="shrink-0 mr-1 w-3 h-3 text-[#12B76A]" />}
                {failed && <AlertCircle className="shrink-0 mr-1 w-3 h-3 text-[#F04438]" />}
